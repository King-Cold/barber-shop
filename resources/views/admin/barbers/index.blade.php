<x-app-layout>
    ...
    <form method="GET" action="{{ route('admin.barbers.index') }}" class="flex flex-col md:flex-row justify-between items-center mb-6 space-y-4 md:space-y-0 bg-white p-4 rounded-xl shadow-sm border border-gray-100 font-sans">
        ...
        <div class="flex flex-col md:flex-row items-center space-y-4 md:space-y-0 md:space-x-4 w-full md:w-auto">
            <div class="relative w-full md:w-80">
                ...
                <input type="text" name="search" value="{{ $search }}" class="bg-white border border-gray-200 text-gray-900 text-sm rounded-lg focus:ring-bronze-gold focus:border-bronze-gold block w-full pl-10 p-2.5 shadow-sm" placeholder="Buscar por nombre o especialidad...">
            </div>
                ...
                <thead>
                    <tr class="bg-gray-50/50 border-b border-gray-100">
                        <th class="px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100 transition-colors">
                            <a href="{{ route('admin.barbers.index', ['sort' => 'id', 'direction' => ($sortField === 'id' && $sortDirection === 'asc') ? 'desc' : 'asc', 'search' => $search, 'per_page' => $perPage]) }}" class="flex items-center">
                                ID
                                @if($sortField === 'id')
                        ...
                        <th class="px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100 transition-colors">
                            <a href="{{ route('admin.barbers.index', ['sort' => 'name', 'direction' => ($sortField === 'name' && $sortDirection === 'asc') ? 'desc' : 'asc', 'search' => $search, 'per_page' => $perPage]) }}" class="flex items-center">
                                Barbero
                                @if($sortField === 'name')
                                    <i class="fa-solid fa-sort-{{ $sortDirection === 'asc' ? 'up' : 'down' }} ml-1.5"></i>
                                @else
                        ...
                        <th class="px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100 transition-colors">
                            <a href="{{ route('admin.barbers.index', ['sort' => 'email', 'direction' => ($sortField === 'email' && $sortDirection === 'asc') ? 'desc' : 'asc', 'search' => $search, 'per_page' => $perPage]) }}" class="flex items-center">
                                Email
                                @if($sortField === 'email')
                                    <i class="fa-solid fa-sort-{{ $sortDirection === 'asc' ? 'up' : 'down' }} ml-1.5"></i>
                                @else
                        ...
                        <th class="px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100 transition-colors">
                            <a href="{{ route('admin.barbers.index', ['sort' => 'phone', 'direction' => ($sortField === 'phone' && $sortDirection === 'asc') ? 'desc' : 'asc', 'search' => $search, 'per_page' => $perPage]) }}" class="flex items-center">
                                Teléfono
                                @if($sortField === 'phone')
                                    <i class="fa-solid fa-sort-{{ $sortDirection === 'asc' ? 'up' : 'down' }} ml-1.5"></i>
                                @else
                        ...
                        <th class="px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100 transition-colors">
                            <a href="{{ route('admin.barbers.index', ['sort' => 'specialty', 'direction' => ($sortField === 'specialty' && $sortDirection === 'asc') ? 'desc' : 'asc', 'search' => $search, 'per_page' => $perPage]) }}" class="flex items-center">
                                Especialidad
                    ...
                    @forelse($barbers as $barber)
                        <tr class="hover:bg-gray-50 transition-colors duration-150">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <a href="{{ route('admin.barbers.records', $barber->id) }}" class="inline-flex items-center justify-center text-vintage-gold hover:text-yellow-600 bg-vintage-gold/10 hover:bg-vintage-gold/20 p-2 rounded-md transition-colors mr-2" title="Ver Historial de Citas">
                                    <i class="fa-solid fa-eye"></i>
                                </a>
                                <a href="{{ route('admin.barbers.schedule', $barber->id) }}" class="inline-flex items-center justify-center text-emerald-600 hover:text-emerald-900 bg-emerald-50 hover:bg-emerald-100 p-2 rounded-md transition-colors mr-2" title="Gestionar Horario de Trabajo">
                                ...
                                <form id="delete-form-{{ $barber->id }}" action="{{ route('admin.barbers.destroy', $barber->id) }}" method="POST" class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="button" onclick="confirmDelete('{{ $barber->id }}', '{{ $barber->name }}')" class="inline-flex items-center justify-center text-red-600 hover:text-red-900 bg-red-50 hover:bg-red-100 p-2 rounded-md transition-colors" title="Eliminar Barbero">
    ...
    <script>
        function confirmDelete(id, name) {
            Swal.fire({
                title: '¿Estás seguro?',
                text: `Estás a punto de eliminar al barbero "${name}". ¡No podrás revertir esto!`,
                icon: 'warning',
